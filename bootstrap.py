...
parser.add_argument("--config-id", default="DevRelease",
                    help="Configuration preset to use (e.g. DevRelease, HW2C_Exe)")
parser.add_argument("--delink", required=False,
                    help="Path to delink executable (skips download)")
parser.add_argument("--objdiff-cli", required=False,
...
def verify_hash(required: bool, path_str: str, expected: str):
    ...
    if actual.casefold() != expected:
        if required:
            ...
            print(f"{path} SHA1 invalid expected:{expected} actual:{actual}")
            return
    else:
...
if args.delink:
    print(f"Using overridden delink: {DELINK_EXE}")
else:
    download_file(DELINK_URL, DELINK_EXE, DELINK_SHA1)

if args.objdiff_cli:
    print(f"Using overridden objdiff-cli: {OBJDIFF_CLI_EXE}")
else:
...
if CONFIG_ID == "DevRelease":
    ...
    for bin_path in ORIG_BINARIES:
        binary_name = bin_path.name
        delink_dir = DELINK_OUTPUT_DIR / binary_name
        delink_dir.mkdir(parents=True, exist_ok=True)
        run([
            str(DELINK_EXE), "macho-split", str(bin_path),
            "--emit-elf",
            "-o", str(delink_dir),
        ])
