...
#pragma warning(default: 25028)
    ...
    void UnRegisterAsyncWaitAndUnloadLibrary(PTP_CALLBACK_INSTANCE instance, PTP_WAIT waiter)
    {
        ...
        CONCRT_COREASSERT(Concurrency::GetOSVersion() >= Concurrency::IResourceManager::Vista);
        // __crtFreeLibraryWhenCallbackReturns has no fallback on XP, but this function is never invoked on XP (as asserted above)
    ...
    PTP_WAIT RegisterAsyncWaitAndLoadLibrary(HANDLE waitingEvent, PTP_WAIT_CALLBACK callback, PVOID data)
    {
        ...
        // __crtCreateThreadpoolWait and __crtSetThreadpoolWait will fail on XP, but this function is never invoked on XP (as asserted above)
    ...
    PTP_TIMER RegisterAsyncTimerAndLoadLibrary(DWORD timeoutms, PTP_TIMER_CALLBACK callback, PVOID data)
    {
        CONCRT_COREASSERT(Concurrency::GetOSVersion() >= Concurrency::IResourceManager::Vista);
        ...
        __crtSetThreadpoolTimer(timer, &time, 0, 0);
        return timer;
    }
