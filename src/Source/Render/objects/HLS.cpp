...
_extern HLSConversion &_sub_1005F420(HLSConversion *const, HLSConversion const &);
_inline HLSConversion &HLSConversion::operator=(HLSConversion const &) // 0x1005F420
{
    mangled_assert("??4HLSConversion@@QAEAAV0@ABV0@@Z");
    compiler_generated();
    todo("implement");
    HLSConversion & __result = _sub_1005F420(this, arg);
    return __result;
}

_extern void _sub_1005F630(float, float, float, float &, float &, float &);
void HLSConversion::RGBToHLS(float R, float G, float B, float &H, float &L, float &S) // 0x1005F630
{
    ...
    todo("implement");
    _sub_1005F630(R, G, B, H, L, S);
}

_extern float _sub_1005F780(float, float, float);
float HLSConversion::colHLSValue(float n1, float n2, float hue) // 0x1005F780
{
    mangled_assert("?colHLSValue@HLSConversion@@SGMMMM@Z");
    todo("implement");
    float __result = _sub_1005F780(n1, n2, hue);
    return __result;
}

_extern void _sub_1005F430(float, float, float, float &, float &, float &);
