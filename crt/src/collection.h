...
namespace Platform {
  namespace Collections {
    namespace Details {
        ...
        template <typename T> inline Agile<T^> MakeWrap(T^ const & t) {
            return Agile<T^>(t);
        }
    ...
    public:
        ...
        virtual void InsertAt(unsigned int index, T item) = WFC_Base::InsertAt {
            try {
                ...
                m_vec->emplace(m_vec->begin() + index, item);
    ...
    namespace Details {
        ...
        template <typename K, typename C> class WrapComp {
        public:
            WrapComp(const C& comp)
                : m_comp(comp) { }

            bool operator()(const Agile<K^>& l, const Agile<K^>& r) const {
                return m_comp(l.Get(), r.Get());
            }
        ...
        template <typename K, typename V, typename C> struct WrapMap<K^, V, C> {
            typedef ::std::map<Agile<K^>, V, WrapComp<K, C>> type;
        };

        template <typename K, typename V, typename C> struct WrapMap<K, V^, C> {
            typedef ::std::map<K, Agile<V^>, C> type;
        };

        template <typename K, typename V, typename C> struct WrapMap<K^, V^, C> {
            typedef ::std::map<Agile<K^>, Agile<V^>, WrapComp<K, C>> type;
        };
        ...
        private:
            typedef typename WrapMap<K, V, C>::type WrappedMap;

        internal:
            IteratorForMapView(const ::std::shared_ptr<unsigned int>& ctr, const ::std::shared_ptr<WrappedMap>& m)
