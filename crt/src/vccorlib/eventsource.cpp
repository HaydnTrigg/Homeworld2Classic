...
namespace Platform { namespace Details {
	...
	public:
		...
		__int64 AddTail(_In_ std::shared_ptr<TDelegate>* element) throw()
		{
			*_end = *element;
			_end++;
			return reinterpret_cast<__int64>(reinterpret_cast<void*>(element->get()));
		}

		static __int64 ExtractCookie(std::shared_ptr<TDelegate> *ptr)
		{
			return reinterpret_cast<__int64>(reinterpret_cast<void*>(ptr->get()));
		}
	private:
	...
	CPPCLI_FUNC void* __stdcall EventSourceGetTargetArrayEvent(void* targets, unsigned int index)
	{
		auto arr = reinterpret_cast<EventTargetArray*>(targets);
